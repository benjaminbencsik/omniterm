...
function Set-DefaultModels {
    ...
    switch ($provider.SelectedItem) {
        'Anthropic' {
            [void]$model.Items.AddRange(@('claude-sonnet-4-5','claude-opus-4-1','claude-haiku-4-5'))
        }
        'Google Gemini' {
            [void]$model.Items.AddRange(@('gemini-2.5-flash','gemini-2.5-pro','gemini-2.0-flash'))
        }
        default {
            [void]$model.Items.AddRange(@('gpt-5-mini','gpt-5','gpt-4.1-mini'))
        }
    }
...
function Invoke-Gemini([string]$apiKey,[string]$modelName) {
    $contents = @($history | ForEach-Object {
        @{ role = $(if ($_.role -eq 'assistant') {'model'} else {'user'}); parts = @(@{ text = $_.content }) }
    })
